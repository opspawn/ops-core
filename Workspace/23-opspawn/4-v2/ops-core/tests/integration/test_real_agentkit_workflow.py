...
# TODO: Define base URLs based on docker-compose service names/ports
...
# Use a fixed agent ID consistent with docker-compose.real-agentkit.yml
TEST_AGENT_ID = "test-sim-agent-001"
...
def docker_compose_up_down():
    ...
    subprocess.run(start_command, check=True, cwd=project_dir)
    ...
    try:
        ...
        subprocess.run(log_command, cwd=project_dir)
    ...
    subprocess.run(stop_command, check=True, cwd=project_dir)
...
@pytest.mark.asyncio
async def test_end_to_end_workflow(opscore_client: httpx.AsyncClient):
    """
    ...
    """
    agent_id = TEST_AGENT_ID # Use the fixed ID

    # 1 & 2: Verify Agent Registration (via webhook)
    print(f"Verifying registration of agent {agent_id} in Ops-Core...")
    try:
        # Pass base URL instead of client fixture instance
        await poll_for_agent_state(OPSCORE_BASE_URL, agent_id, "idle", timeout=45)
        print(f"Agent {agent_id} successfully registered in Ops-Core.")
    except TimeoutError:
        pytest.fail(f"Agent {agent_id} did not appear in Ops-Core after startup.")
    ...
    trigger_response = await opscore_client.post(f"/v1/opscore/agent/{agent_id}/workflow", json=workflow_payload)
    assert trigger_response.status_code == 200 # Or 202 if async trigger
    print(f"Workflow triggered successfully for agent {agent_id}.")
    # TODO: Potentially extract session ID if needed

    # 4, 5, 6: Verify Agent becomes 'active'
    print(f"Waiting for agent {agent_id} to become active...")
    try:
        # Pass base URL instead of client fixture instance
        await poll_for_agent_state(OPSCORE_BASE_URL, agent_id, "active", timeout=30)
        print(f"Agent {agent_id} is now active.")
    except TimeoutError:
        pytest.fail(f"Agent {agent_id} did not become active.")

    # 7: Verify Agent becomes 'idle' again
    print(f"Waiting for agent {agent_id} to become idle...")
    try:
        # Pass base URL instead of client fixture instance
        await poll_for_agent_state(OPSCORE_BASE_URL, agent_id, "idle", timeout=30)
        print(f"Agent {agent_id} is now idle. Workflow complete.")
    except TimeoutError:
        pytest.fail(f"Agent {agent_id} did not return to idle.")
