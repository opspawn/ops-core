...
async def agent_notification(payload: AgentNotificationPayload):
    ...
    """
    logger.info(f"API: Received agent notification: Event='{payload.event_type}', AgentName='{payload.agent_details.agentName}'")
...
async def update_agent_state(
    ...
    """
    logger.info(f"API: Received state update for agent {agent_id}: State='{state_update.state}' Timestamp='{state_update.timestamp}'")

    # Pydantic performs basic validation. Explicit check for path/payload ID match:
    if agent_id != state_update.agentId:
        logger.warning(f"API: Agent ID mismatch in state update: Path='{agent_id}', Payload='{state_update.agentId}'")
        # Raise HTTPException directly for validation errors
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    logger.info(f"API: Successfully processed state update for agent {agent_id}")
...
async def get_agent_state(agent_id: str):
   ...
   Requires API Key authentication.
   """
   logger.info(f"API: Received request for current state of agent {agent_id}")
...
async def trigger_workflow(
    ...
    """
    logger.info(f"API: Workflow trigger request received for agent {agent_id}")
    ...
    try:
        if trigger_request.workflowDefinitionId:
            ...
            logger.debug(f"API: Attempting to use existing workflow definition ID: {workflow_id}")
            ...
            logger.debug("API: Using inline workflow definition from request.")
            ...
            try:
                ...
                logger.error(f"API: Invalid inline workflow definition provided: {validation_error}", exc_info=True)
            ...
            if not existing_def:
                logger.info(f"API: Saving new inline workflow definition: {workflow_id}")
            ...
            logger.error("API: Workflow trigger request missing both definition ID and inline definition.")
        ...
        if not workflow_def or not workflow_id:
            ...
            logger.error("API: Internal error: Failed to resolve workflow definition or ID.")
        ...
        logger.warning(f"API: Workflow definition not found: {e}")
    ...
    try:
        ...
        logger.debug(f"API: Calling workflow.enqueue_task for session {session.sessionId}")
        workflow.enqueue_task(task_instance_data)
        logger.info(f"API: Enqueued first task {task_id} for session {session.sessionId}")

    except Exception as e: # Catch potential errors during enqueue *after* session started
        error_msg = f"API: Failed to enqueue first task after starting session {session.sessionId}: {e}"
