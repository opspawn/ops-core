"""
Module for updating atomic charges based on force-field type mapping.
...
from .legacy.update_charges import update_car_charges as legacy_update_car
from .legacy.update_charges import update_mdf_charges as legacy_update_mdf
from .legacy.update_charges import load_mapping

logger = logging.getLogger(__name__)

def update_charges_obj(system: System, mapping: Union[Dict, str]) -> int:
    """
    Update atomic charges in a System object based on a mapping.
    ...
    return system.update_charges(mapping)

def update_charges(car_file: Optional[str] = None, 
    ...
    """
    Updates atomic charges in CAR and/or MDF files or in a System object.
    ...
    if object_mode:
        ...
        # Update the system
        update_count = update_charges_obj(system, mapping)
    ...
    # Legacy file-based approach
        ...
        if car_file and output_car:
            logger.info(f"Updating charges in CAR file: {car_file}")
            car_updates = legacy_update_car(car_file, output_car, mapping)
            results['car_updates'] = car_updates
            logger.info(f"Updated {car_updates} charges in CAR file: {output_car}")
        
        # Update MDF file if provided
        if mdf_file and output_mdf:
            logger.info(f"Updating charges in MDF file: {mdf_file}")
            mdf_updates = legacy_update_mdf(mdf_file, output_mdf, mapping)
            results['mdf_updates'] = mdf_updates
            logger.info(f"Updated {mdf_updates} charges in MDF file: {output_mdf}")
